...
- （可选）如果希望用 AI 生成报告摘要，需要 Anthropic API Key（[console.anthropic.com](https://console.anthropic.com) 注册获取，按官方当前模型价格和用量计费）。不配置也可以，脚本会用 Python 字符串拼接生成报告

## 项目定义卡

| 维度 | 本案例答案 |
| --- | --- |
| 适用人群 | 有固定数据源和固定报告格式，想减少手动汇总和飞书发送的人 |
| 输入材料 | 数据源、指标口径、报告模板、飞书 Webhook、运行频率、敏感凭证管理方式 |
| 最小可行版本 | 本地 Python 脚本读取 CSV/Excel 或导出文件，生成文字报告并推送飞书 |
| 风险边界 | 首次必须人工核对数字；Webhook、App Secret、API Key 只能走环境变量或 Secrets |
| 验收结果 | 手动运行脚本后飞书收到报告，核心数字与人工核对一致，再配置定时执行 |
...
这是低门槛的飞书接入方式。飞书开放平台说明，自定义机器人可通过 webhook 地址向群聊推送消息；是否需要额外审批、安全设置和限流以官方当前文档和你的组织权限为准。
...
> 建议先用 CSV 或 Excel 本地文件跑通整个流程，确认报告格式和发送都没问题后，再接飞书 API。飞书应用、权限和数据访问范围可能需要管理员或租户审批，耗时以你所在组织当前流程为准，不要让它阻塞第一版。
...
⚠️ 注意：创建企业自建应用、发布版本、申请权限和开通数据权限通常需要飞书管理员或应用管理员协助。如果你不是管理员，需要提前确认组织内审批流程。
...
**方案 B：GitHub Actions（云端运行，推荐用于非严格准点任务）**
...
on:
  schedule:
    # GitHub Actions schedule 使用 UTC；北京时间周一 9:00 对应 UTC 周一 1:00。
    # 如果你的目标时区不是 Asia/Shanghai，请先换算成 UTC。
    - cron: '0 1 * * 1'
...
**优点**：不需要自己的服务器，有运行日志，也可以手动触发测试。GitHub 托管 runner 的免费额度、私有仓库计费和超额规则，以 GitHub Actions 当前计费文档为准。

⚠️ GitHub Actions 官方文档说明，schedule 事件在高负载时可能延迟，负载足够高时部分排队任务甚至可能被丢弃；公开仓库长时间无活动时，定时 workflow 也可能被自动禁用。如果你的报告对发送时间精度要求高（比如必须 9:00 准时），考虑腾讯云函数或自建服务器。对于一般周报场景，这个风险通常可以接受。
...
飞书自定义机器人 Webhook 有频率和安全限制。正常报告推送一般不会触发，但调试时如果在循环里连续发送，可能会被临时限流。具体阈值以飞书开放平台当前说明为准，调试时建议每次只发一条测试消息。
