...
- 能在真实目录里启动 Claude Code 或打开合适的 Desktop/Cowork 工作区
- 能选择保守权限模式，完成一次只读验证而不改动文件
- 能记录工具读到了什么、没读到什么、哪里需要人工判断
...
说明：
- 官方安装页也提供 native binary 等路径，但这些路径的可用状态会随平台更新变化
- Windows 建议按官方当前说明使用 WSL 或 Git Bash，先不要把 PowerShell/CMD 当默认路径
- 如果团队内统一安装方式，以官方文档和组织 IT 策略为准
...
- 你的账号与工作区当前支持 Cowork，具体可用性以官方当前页面为准
...
补充：
- Auto Mode 是单独的自动化能力，不等于权限模式；是否可用、如何启用以官方当前文档为准
...
完成上面任意一个验证任务，截图保存，并补一份验证记录：

| 项目 | 记录 |
|------|------|
| 使用入口 | CLI / IDE / Desktop / Web / Cowork |
| 工作目录 | 只写目录名，不记录敏感路径 |
| 权限模式 | default / plan / 其他 |
| Claude 读到的文件 | 列 3-5 个代表文件 |
| 输出是否准确 | 准确 / 部分准确 / 不准确 |
| 下一步 | 继续只读 / 小范围编辑 / 先补上下文 |
...
- [ ] 能在真实目录里完成一次只读分析任务
- [ ] 能说出 `default`、`plan`、`acceptEdits`、`bypassPermissions` 的基本区别和适用边界
- [ ] 验证记录里包含读取范围、输出准确性、风险点和下一步动作
