...
- 能为一个外部工具判断是否值得接 MCP
- 能按最小权限配置 1 个 MCP，并用只读任务验证
- 能写出授权范围、Token 保管方式、撤销方式和失败处理
...
- **Streamable HTTP**：当前规格里的远程服务主路径
- **stdio**：本地进程，适合文件系统、数据库、脚本工具
- **HTTP+SSE**：老服务器仍可能支持，作为兼容路径看待，不再当新接入的首选

### 已有官方/社区 MCP 服务器列表

下面是常见类型示例，不代表固定推荐清单；具体服务器、安装方式、权限范围和维护状态以对应官方或当前项目文档为准。
...
2. 先判断 transport：远程服务优先 Streamable HTTP；本地工具再用 stdio；HTTP+SSE 只有在服务器只提供旧接口时才用
...
- **不要把所有 MCP 都理解成“本地插件”**：远程 Streamable HTTP server 很常见
...
- **SSE 只作兼容**：如果服务同时支持 Streamable HTTP 和 HTTP+SSE，优先选 Streamable HTTP
...
|------|------|
| MCP 名称 |  |
| transport | Streamable HTTP / stdio / 兼容 SSE |
| 授权范围 | 只写必要目录、repo 或 scope |
| Token 保存方式 | 环境变量 / 密钥管理 / OAuth |
| 验证任务 | 先用只读任务 |
| 撤销方式 | 删除配置、撤销 token 或关闭服务 |
| 风险点 | 误读、误写、外发、权限过大 |
...
3. **把 transport 选反了**：远程服务硬接 stdio，或者明明有 Streamable HTTP 还去用旧 SSE。先看服务器支持什么，再选最稳的 transport。
4. **以为 MCP 只有一种运行方式**：有些 server 跑在你本机，有些是远程 HTTP 服务。别把“本地要保持进程运行”和“远程服务可用”混成一个概念。

## 验收标准

- [ ] 能独立接入 1 个 MCP 并验证是否生效
- [ ] 能说出配置步骤、授权范围、撤销方式和 Token 保管方式
- [ ] 能完成一个只读小任务，并解释为什么没有开放更大权限
