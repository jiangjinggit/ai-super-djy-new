...
- 能把一个重复任务从手动流程升级为 command/skill，再评估是否适合调度
- 能区分 Hook、`/loop`、`/schedule`、Desktop scheduled tasks 和 Auto Mode 的边界
- 能为自动化任务写出停止条件、失败通知、人工接管和回退方式
...
下面示例用于说明 Hook 结构。正式接入外部通知前，先在本地日志里验证 payload 和触发条件，确认不会泄露敏感命令或文件路径。
...
**永远不要**：跳过手动验证阶段直接上全自动化。没有验证过的流程自动化后出问题，排查成本是手动的十倍。
...
| 项目 | 必填内容 |
|------|----------|
| 手动验证次数 | 至少 3 次，记录失败案例 |
| 输入来源 | 文件、仓库、接口或连接器 |
| 输出格式 | 固定格式和保存位置 |
| 停止条件 | 读不到输入、验证失败、diff 超范围、外部请求异常 |
| 人工确认点 | 删除、安装、推送、改配置、发请求 |
| 回退方式 | 恢复文件、撤销 token、关闭 schedule 或删除 hook |
