...
- 服务器 + 模型 + 飞书的选型决策表（含国内/国外模型怎么选）
- 一条 30 分钟从零到飞书日报的最短路径
- 踩坑清单和社区资源
...
> 📖 [博客园 — 保姆级飞书对接教程](https://www.cnblogs.com/catchadmin/p/19556552)（Linux 安装 + 飞书机器人全流程）
...
```json
{
  "cron": [{
    "pattern": "30 8 * * 1-5",
    "prompt": "生成今日工作日报，整理重要事项和日程安排，发送到飞书。",
...
```bash
openclaw run --prompt "生成一份测试日报，发送到飞书"
```
