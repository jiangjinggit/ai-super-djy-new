...
export const MODULE_CONTENT: Record<ModuleId, ModuleContent> = {
  ...
  "llm": {
    ...
    icon: Brain,
    color: "purple",
    description: "基于 OpenAI、Anthropic、DeepSeek、月之暗面官方文档，深度对比 2026 年主流大模型的上下文窗口、价格和能力边界。帮你为智能体系统选配最强\"大脑\"。",
    keyTakeaways: [
    ...
    sections: [
      ...
      { title: "智能体配置实战", content: "获取 API Key，在智能体平台中配置模型，测试对话效果。", icon: Cpu }
    ],
    lessons: [
      {
      ...
      {
        title: "API 注册与智能体平台配置实战",
        content: "手把手教你注册 OpenAI / Anthropic / DeepSeek / 月之暗面的开发者账号，获取 API Key，并在智能体平台中完成配置。",
    ...
    cta: { text: "选好模型了？前往智能体配置实战 → 学习 AI 智能体实战", link: "/module/agents" }
  },
  "agents": {
    title: "AI 智能体实战专区",
    subtitle: "OpenClaw + Claude Code 双引擎",
    icon: Cpu,
    color: "emerald",
    description: "聚焦 AI 智能体的核心能力与落地实践，重点覆盖 OpenClaw 与 Claude Code 两条主线。前者偏本地自治和长期自动化，后者偏多端协作与任务执行，组合后可形成可复用的超级个体工作流。",
    keyTakeaways: [
      "OpenClaw：本地部署 + 多渠道接入 + Skills 自动化，适合稳定运行后台任务",
      "Claude Code：不只写代码，也可用于文档产出、排障协作、任务编排与远程执行",
      "双平台组合：OpenClaw 负责长期自治，Claude Code 负责复杂任务拆解与快速迭代",
      "落地原则：先做权限边界与审批关卡，再放大自动化规模"
    ],
    sections: [
      { title: "OpenClaw 自治执行", content: "本地部署、全渠道接入、Skills 自动化，适合 24/7 后台任务与流程托管。", icon: Rocket },
      { title: "Claude Code 协作执行", content: "终端+代码库+文档一体执行，支持渠道协作、远程控制与计划任务，不止编码场景。", icon: Code },
      { title: "双引擎编排", content: "以 OpenClaw 保持稳定自动化，以 Claude Code 处理高变化任务，实现“长期运行 + 快速迭代”。", icon: Layers }
    ],
    lessons: [
      {
      ...
      {
        title: "Claude Code 最新能力：从代码助手到通用执行伙伴",
        content: "Claude Code 已不再局限于写代码。它可以在真实项目上下文中完成文档整理、故障排查、发布前检查、跨终端协作等任务。",
        image: "https://picsum.photos/seed/claude-code-agent/800/450",
        details: [
          "代码与文档双场景：既能改代码，也能做项目说明、发布日志、排障记录",
          "平台与集成：可在本地终端、IDE、Slack 等入口协作处理任务",
          "远程控制：可在浏览器/移动端远程驱动本地会话，持续跟进执行状态",
          "计划任务：适合日更巡检、日志汇总、日报草稿等周期性任务",
          "工程与运营一体：从 PR 检查到知识库更新，都可在同一执行链里完成",
          "角色定位：更像执行型工作台，而非单一“写代码机器人”"
        ],
        fullContent: [
          { subtitle: "1. 为什么说不只是代码", text: "Claude Code 的核心价值是“在真实上下文里执行任务”，代码只是其中一种。你可以把仓库文档、任务清单、报错日志、发布信息一起交给它，要求它输出结构化结论或可直接使用的交付物。例如：发布前检查清单、变更说明、值班排障摘要、日报周报草稿等。" },
          { subtitle: "2. 多端协作与远程执行", text: "Claude Code 支持多入口协作，不一定局限在本地 IDE。你可以在终端处理复杂任务，在 Slack 等协作入口同步进度，也可以通过远程控制方式在外部设备继续驱动本地会话，适合“在路上继续处理任务”的高频场景。" },
          { subtitle: "3. 计划任务与流程编排", text: "对于固定频率的工作，Claude Code 可用于周期化执行，例如：每天抓取构建日志并总结风险、每周汇总 Issue 状态、每次发布后自动整理变更说明。建议结合权限隔离和人工确认关卡，避免自动化误触发高风险动作。" }
        ]
      },
      {
        title: "OpenClaw + Claude Code 选型与组合策略",
        content: "两者不是替代关系，而是分工关系：OpenClaw 保持稳定自治，Claude Code 负责高变化任务与协作执行。",
        image: "https://picsum.photos/seed/agent-stack-choice/800/450",
        details: [
          "优先 OpenClaw：你需要本地部署、全渠道消息接入、7x24 稳定后台任务",
          "优先 Claude Code：你需要快速改造代码库、产出文档、跨端协作执行任务",
          "组合建议：OpenClaw 跑长周期流程，Claude Code 处理临时复杂需求",
          "与 Manus 的关系：Manus 偏目标驱动的自主执行，可做实验与补充，不建议先重投入",
          "上线顺序：先做最小闭环，再扩展到多流程并行，最后再加自动化密度",
          "治理原则：动作分级、权限分级、成本分级，避免工具能力先于流程设计"
        ],
        fullContent: [
          { subtitle: "1. 什么时候先上 OpenClaw", text: "如果你的任务以重复流程为主，例如邮件整理、监控告警、跨渠道通知，OpenClaw 的稳定自治能力价值更高。你可以先用 1-2 条固定流程试运行，验证稳定性后再逐步扩容。核心是“长期可靠”，不是一次性惊艳。" },
          { subtitle: "2. 什么时候先上 Claude Code", text: "如果你的核心瓶颈是需求变化快、任务跨代码和文档、需要多人协作同步，那么先把 Claude Code 接入日常流程更合适。它在代码修改、变更解释、技术沟通、排障追踪上的联动效率更高，适合快速迭代阶段。" },
          { subtitle: "3. 双引擎组合打法", text: "推荐做法是：OpenClaw 负责“可标准化”的长期流程，Claude Code 负责“需判断”的复杂任务。比如 OpenClaw 定时拉数据并触发任务，Claude Code 根据上下文生成决策草案或改动方案，最后由你做人工确认并发布。" }
        ]
      },
      {
        title: "超级个体实战工作流：双主线落地模板",
        content: "基于 OpenClaw + Claude Code 的能力，设计可直接落地的 5 个高频场景模板，覆盖内容、开发、运营三类主任务。",
        image: "https://picsum.photos/seed/agent-workflow-pack/800/450",
        details: [
          "工作流 1：邮件与消息中台 — OpenClaw 自动归集 + Claude Code 输出回复草稿",
          "工作流 2：内容生产线 — OpenClaw 抓热点与分发，Claude Code 生成结构化稿件",
          "工作流 3：发布前巡检 — Claude Code 汇总风险，OpenClaw 触发多渠道提醒",
          "工作流 4：竞品与舆情监控 — OpenClaw 定时抓取，Claude Code 生成差异解读",
          "工作流 5：知识库维护 — OpenClaw 做定时归档，Claude Code 做文档结构优化"
        ],
        fullContent: [
          { subtitle: "1. 运营沟通自动化", text: "OpenClaw 定时读取邮箱、客服渠道、社群消息，先按优先级聚合并去重，再把高优先级问题交给 Claude Code 生成结构化回复建议。你只需审核关键回复并一键发送，日常沟通耗时可明显下降。" },
          { subtitle: "2. 内容与发布联动", text: "OpenClaw 负责热点抓取、素材收集和定时分发；Claude Code 负责把多来源信息整理成可发布文本（文章提纲、FAQ、更新说明）。这种分工可以把“收集信息”和“结构化表达”拆开，提高单人产能稳定性。" },
          { subtitle: "3. 监控与复盘闭环", text: "OpenClaw 定时抓取竞品与业务指标，Claude Code 生成变化摘要、风险判断和下一步建议，再推送到你的工作群或任务板。建议保留人工审批节点，用于控制错误扩散和资源浪费。" }
        ]
      }
    ],
