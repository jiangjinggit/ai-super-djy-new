  ...
  return (
    <motion.article initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="pt-32 pb-24 px-6 max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-4xl md:text-6xl font-black text-white leading-tight mb-5">{lesson.title}</h1>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="rounded-2xl border border-white/10 bg-white/5 px-5 py-4">
          <p className="text-xs uppercase tracking-[0.2em] text-gray-500 mb-2">难度</p>
          <p className="text-white font-semibold">{DIFFICULTY_LABELS[lesson.difficulty]}</p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-white/5 px-5 py-4">
          <p className="text-xs uppercase tracking-[0.2em] text-gray-500 mb-2">预计时长</p>
          <p className="text-white font-semibold inline-flex items-center gap-2">
            <Clock3 size={16} className="text-blue-300" /> {lesson.estimatedTime}
          </p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-white/5 px-5 py-4">
          <p className="text-xs uppercase tracking-[0.2em] text-gray-500 mb-2">课程进度</p>
          <p className="text-white font-semibold inline-flex items-center gap-2">
            <Layers3 size={16} className="text-blue-300" /> 第 {lessonIndex + 1} / {moduleContent.lessons.length} 课
          </p>
        </div>
      </div>

      <div className="mb-10 rounded-[28px] border border-white/10 bg-black/20 px-6 py-5">
        <p className="text-xs uppercase tracking-[0.2em] text-gray-500 mb-3">本课目标</p>
        <p className="text-lg text-gray-200 leading-relaxed inline-flex items-start gap-3">
          <Target size={18} className="text-blue-300 shrink-0 mt-1" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {prevLesson ? (
            <Link
              ...
              className="rounded-2xl border border-white/10 bg-black/20 px-5 py-4 hover:bg-white/5 transition-colors"
            >
              <p className="text-xs uppercase tracking-[0.2em] text-gray-500 mb-2">上一课</p>
              <p className="text-white font-semibold inline-flex items-center gap-2">
                <ArrowLeft size={16} className="text-blue-300" /> {prevLesson.title}
              </p>
            </Link>
          ) : (
            <div className="rounded-2xl border border-white/10 bg-black/10 px-5 py-4 text-gray-500">已经是本模块第一课</div>
          )}

          {nextLesson ? (
            <Link
              to={`/module/${moduleId}/lesson/${nextLesson.slug}`}
              className="rounded-2xl border border-white/10 bg-black/20 px-5 py-4 hover:bg-white/5 transition-colors text-left"
            >
              <p className="text-xs uppercase tracking-[0.2em] text-gray-500 mb-2">下一课</p>
              <p className="text-white font-semibold inline-flex items-center gap-2">
                {nextLesson.title} <ArrowRight size={16} className="text-blue-300" />
              </p>
            </Link>
          ) : (
            <div className="rounded-2xl border border-white/10 bg-black/10 px-5 py-4 text-gray-500">已经是本模块最后一课</div>
          )}
        </div>
      </div>
    </motion.article>
